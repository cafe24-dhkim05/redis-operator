...
Redis standalone configuration can be customized by [values.yaml](https://github.com/cafe24-dhkim05/helm-charts/blob/main/charts/redis/values.yaml). The recommended way of managing the setup is using `helm` but if the setup is not maintained by it, `YAML` CRD parameters can be modified in the manifest.
...
| `redisStandalone.image`           | docker.io/cafe24dhkim05/redis          | Name of the redis image                                                                       |
...
| `redisExporter.image`             | docker.io/cafe24dhkim05/redis-exporter | Name of the redis exporter image                                                              |
